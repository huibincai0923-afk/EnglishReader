...
/* V3_2_MARKING_PATCH: manual marking is independent of vocabulary. */
(function(){
  const KEY="reader-mark-color-v32";
  let currentColor=localStorage.getItem(KEY)||"yellow";

  function installMarking(view){
    const doc=view?.document, win=doc?.defaultView;
    if(!doc||!win||doc.__v32MarkingInstalled)return;
    doc.__v32MarkingInstalled=true;

    const st=doc.createElement("style");
    st.textContent=
      "html,body{background:#f5f0e6!important;}"+
      ".v32-user-mark{padding-bottom:1px;}"+
      ".v32-mark-red{border-bottom:3px solid #c98b8b!important;}"+
      ".v32-mark-yellow{border-bottom:3px solid #d5c58b!important;}"+
      ".v32-mark-blue{border-bottom:3px solid #8eabc0!important;}"+
      ".v32-mark-purple{border-bottom:3px solid #a694b8!important;}"+
      ".v32-mark-green{border-bottom:3px solid #8eaa8e!important;}";
    doc.head?.appendChild(st);

    const palette=doc.createElement("div");
    palette.className="v32-mark-palette hidden";
    ...
    const sync=()=>palette.querySelectorAll("button").forEach(b=>b.classList.toggle("selected",b.dataset.c===currentColor));
    const hide=()=>{palette.classList.add("hidden");range=null};
    const show=()=>{
      const sel=win.getSelection();
      if(!sel||sel.rangeCount===0||sel.isCollapsed||!sel.toString().trim())return;
      const r=sel.getRangeAt(0);
      if(!doc.body.contains(r.commonAncestorContainer))return;
      range=r.cloneRange(); sync(); palette.classList.remove("hidden");
      const rect=r.getBoundingClientRect(), w=palette.offsetWidth||190, h=palette.offsetHeight||44;
      palette.style.left=Math.max(8,Math.min(win.innerWidth-w-8,rect.left+rect.width/2-w/2))+"px";
      palette.style.top=Math.max(8,rect.top-h-10)+"px";
    };
    const schedule=()=>setTimeout(show,30);
    ...
    function mark(r,color){
      const walker=doc.createTreeWalker(doc.body,NodeFilter.SHOW_TEXT), nodes=[];
      while(walker.nextNode()){
        const n=walker.currentNode;
        if(!n.nodeValue.trim()||n.parentElement?.closest("script,style,.v32-mark-palette"))continue;
        try{if(r.intersectsNode(n))nodes.push(n)}catch(e){}
      }
      let ok=false;
      nodes.forEach(n=>{
        let a=n===r.startContainer?r.startOffset:0, b=n===r.endContainer?r.endOffset:n.nodeValue.length;
        a=Math.max(0,Math.min(a,n.nodeValue.length)); b=Math.max(0,Math.min(b,n.nodeValue.length));
        if(a>=b)return;
        const f=doc.createDocumentFragment();
        if(a)f.appendChild(doc.createTextNode(n.nodeValue.slice(0,a)));
        const span=doc.createElement("span"); span.className="v32-user-mark v32-mark-"+color;
        span.textContent=n.nodeValue.slice(a,b); f.appendChild(span);
        if(b<n.nodeValue.length)f.appendChild(doc.createTextNode(n.nodeValue.slice(b)));
        n.parentNode.replaceChild(f,n); ok=true;
      });
      return ok;
    }
    ...
    palette.addEventListener("click",e=>{
      const btn=e.target.closest("button"); if(!btn||!range)return;
      currentColor=btn.dataset.c; localStorage.setItem(KEY,currentColor); sync();
      if(mark(range.cloneRange(),currentColor)&&typeof toast==="function")toast("Marked");
      win.getSelection()?.removeAllRanges(); hide();
    });
    doc.addEventListener("mousedown",e=>{if(!palette.contains(e.target))hide()},true);
  }

  function hook(){
    if(typeof book==="undefined"||!book?.rendition)return;
    if(!book.__v32MarkingHooked){
      book.__v32MarkingHooked=true;
      book.rendition.hooks.content.register(installMarking);
      book.rendition.on("rendered",()=>{try{book.rendition.getContents().forEach(installMarking)}catch(e){}});
    }
    try{book.rendition.getContents().forEach(installMarking)}catch(e){}
  }
  let tries=0; const t=setInterval(()=>{hook();if(++tries>80)clearInterval(t)},250);
})();
